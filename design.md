...
>
> 最新约定：新房间固定 9 张初始模板（狼人 2、村民 2、其他角色各 1）；加入最多 10 人。板子增减只编辑草稿，显式提交后才更新公共板子，不读本地板子缓存。多人狼同无操作身份一样，从模板可操作角色中随机分配伪装界面；仅独狼执行真实窥视。揭晓用 `action_was_fake` 提示伪装操作不生效。下文旧版示例若不一致，以本约定和端点文档为准。
...
`Player` 保存：`room`、`userid`、`userpsw`、`avatar`、初始身份 `role`、仅无真实行动身份使用的 `fake_role`、原始操作 `choice`、原始投票 `vote_target`。

- `choice={}` 表示未提交；截止时为缺失或非法操作随机补全合法选择，之后冻结。
- `vote_target=null` 表示未投；`""` 表示弃权；其他值为被投玩家 ID。
- 真实行动角色不重复保存展示身份，展示身份为 `fake_role or role`。
...
所有人都必须操作，禁止主动跳过。无真实行动者获得假身份界面，假操作不改变任何牌；未提交操作由服务端随机补全。必须等待完整操作时限，截止后拒绝新增或覆盖操作。
...
- **身份伪装逻辑**（核心，杜绝场外；**人人必点**）：
  ...
  - 你的真实身份**无行动**（爪牙 / **失眠者** / **村民**）→ 系统**随机从 {预言家, 强盗, 捣蛋鬼, 独狼} 挑一个**身份，给出和真的一样的界面（例如“你可能是捣蛋鬼 🃏，请选择要交换的两名玩家”），照常让你选——作为防窥探摆设。
