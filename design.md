...
3. **操作完成后统一揭晓，然后接着投票。**
   阶段一里每个人都能看到自己**正在操作的身份**，并做出**有真实身份依据的选择**——真捣蛋鬼在阶段一就被告知“你是捣蛋鬼，请选两张”，真预言家被告知去看谁；但**阶段一不显示任何“结果”**：看不到你窥视到的牌面、看不到换牌后的牌面，只是做选择。初始身份在阶段一向本人显示，夜间信息与行动结果**在第二个 View（阶段二）揭晓**——预言家看到他阶段一选的牌、捣蛋鬼看到自己确实换了牌、村民进入讨论；**同一阶段二界面里就给出投出处决对象的控件（每人只能投一次）**。全员投完后，第三 View 展示最终结果。
...
- 行动阶段返回初始身份 `role`，按初始牌与狼人数量推导 `requires_action`。
